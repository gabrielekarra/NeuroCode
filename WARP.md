...
-  - `check` – run structural checks on a Python file (stub).
-  - `patch` – apply an IR-informed patch to a Python file given a high-level fix description (stub).
...
- Run structural checks on a Python file (stub):

  ```bash
  neurocode check path/to/file.py
  ```

- Apply an IR-informed patch (stub) to a Python file using a high-level fix description:

  ```bash
  neurocode patch path/to/file.py --fix "High-level description of the desired change"
  ```

Until the underlying engine is implemented, these commands print stub messages indicating the intended phase of the pipeline.
